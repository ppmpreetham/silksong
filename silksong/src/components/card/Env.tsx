...
const Env = () => {
  ...
  useGSAP(() => {
    ...
    if (card1Ref.current && card2Ref.current && card3Ref.current) {
      ...
      // gsap.to(card3Ref.current.rotation, { z: -1 * PI, duration: 5, ease: 'none' })
      // tl.fromTo(card1Ref.current.position, { z: 20 }, { z: -20, duration: 3, ease: 'none' })
      tl
  ...
  }, { dependencies: [meshesLoaded] });
  
  
  return (
    <div className="h-[200vh] w-screen" ref={container}>
      <Canvas camera={{ position: [0, 0, 50], fov: 50 }} shadows>
        <ambientLight intensity={1.5} />
        {/* <spotLight position={[0, 0, 3]} angle={PI} intensity={20}/> */}
        {/* <pointLight position={[-1, 5, 3]} intensity={30} /> */}
